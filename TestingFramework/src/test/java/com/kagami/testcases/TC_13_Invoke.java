...
public class TC_13_Invoke {
	...
	public String workBookName =  "./TestData/TC_13_Invoke.xlsx";
...
/*	@Test(priority = 4)
	...
	@Test(priority = 6)
	public void processCreationForInvoke()
	{
		test = extent.startTest("Process Creation: Test Suite", "Create Process in Studio....");	
		processCreation = new ProcessCreation(driver);
		processCreation.processCreationInvoke(test, workBookName);
		extent.endTest(test);
		extent.flush();
	}
	...
	@Test(priority = 7)
	public void roleMapping() throws InvalidFormatException, IOException, InterruptedException
	{
		extent = ExtentManager.Instance();
		extent.loadConfig(new File("./extent/config.xml"));
		test = extent.startTest("Role Mapping", "Role Mapping....");
		studioCommonMethods = new StudioCommonMethods(driver);
		studioCommonMethods.mapRoles(test, "CTO");
		extent.endTest(test);
		extent.flush();
	}
