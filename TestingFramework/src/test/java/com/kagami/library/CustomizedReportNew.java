...
Objective: This class will generate customized report and configures Email with report attachment
...
public class CustomizedReportNew implements IReporter{
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,String outdir) {
		...
		String details = testEnvironmentDetails("Firefox", "http://52.37.98.209:8080/kagami-erp-generated/", "Windows7", "1.0.0");
	...
	private void sendMailwithReport(String style, String report, String details) throws AddressException, MessagingException {
		...
		//to zip a file		  
		File file = new File("./test-output/custom-report.html");
	...
	private String testEnvironmentDetails(String browser, String url, String os, String build) {
		String testEnvironmentDetails = 
				"<div class=row>"
						...
						+"</tr>"
						+environmentDetailsRow("Browser", browser)
						+environmentDetailsRow("URL", "<a href=\""+url+"\">"+url+"</a>")
						+environmentDetailsRow("OS", os)
						+environmentDetailsRow("BUILD", build)
						+"</table>"
						+"</div>"
						+"</div>";

		writer.println(testEnvironmentDetails);
	...
	public String rowGenerateSuiteSummaryReport(String suiteName, int pass, int fail, int skip){
		return "<tr>"
				+ "<th>" + suiteName + "</th>"
				+ "<td class=\"text-right\">" + (pass + fail + skip) + "</td>"
				+ "<td class=\"text-right\">" + (pass + fail) + "</td>"
	...
	public String totalGenerateSuiteSummaryReport(int pass, int fail, int skip){
		return "<tr class=\"active\">"
				...
				+ "<td class=\"text-right\">" + (pass + fail + skip) + "</td>"
				+ "<td class=\"text-right\">" + (pass + fail) + "</td>"
				+ "<td class=\"text-right\" "+(pass > 0 ? "style='background-color:#2ECC71'": "") +">" + pass + "</td>"
				+ "<td class=\"text-right\" "+(fail > 0 ? "style='background-color:#E74C3C'": "") +">" + fail + "</td>"
				+ "<td class=\"text-right\" "+(skip > 0 ? "style='background-color:#FFCE56'": "") +">" + skip + "</td>"
				+ "</tr>";	
	}
