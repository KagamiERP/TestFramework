			...
			<td>
				...
				<table style = "border:1px solid black;" border = "1" cellpadding = "5" align="center" class="summary">
					...
					<tr>
						...
						<td align="center"><output><a href="#" onClick="Total();"><xsl:value-of select='$PCI_SS+$PCIYes+$PCINo+$PCI_NSS+$DukeTreadmill+$RaoBleeding+$PCI_TimeOut+$PCI_Mayo+$PCI_MRCD+$PCI_NCDR'/></a></output></td>
						...
						<td align="center"><output><a href="#" onClick="TotalFail();"><xsl:value-of select='$PCI_SSFail+$PCIYesFail+$PCINoFail+$PCI_NSSFail+$DukeTreadmillFail+$RaoBleedingFail+$PCI_TimeOutFail+$PCI_MayoFail+$PCI_MayoFail+$PCI_NCDRFail'/></a></output></td>
					</tr>
		...
		<xsl:variable name="num31"><xsl:value-of select="position()"/></xsl:variable>
			<xsl:for-each select='TestScript[contains(@name,"PCI_Mayo")]'>
				<xsl:variable name="num32"><xsl:value-of select="$num31" /><xsl:value-of select="position()" /></xsl:variable>
					...
					<tr style="display:none;"><xsl:attribute name="class">panel13<xsl:value-of select="$num32"/></xsl:attribute>
						<td colspan="5">
						<div style="display:none;"><xsl:attribute name="id">panel13<xsl:value-of select="$num32"/></xsl:attribute>
						<xsl:for-each select="Iterator">
							<xsl:variable name="num33"><xsl:value-of select="$num32"/><xsl:value-of select="position()" /></xsl:variable>
							<table border=".5" align="center" class="test">
								<tr>
									<td align="center" colspan="5" class="flip13" ><xsl:attribute name="id"><xsl:value-of select="$num33"/></xsl:attribute><font face="Calibri" size="2" color="#000000"><b>Iteration : <xsl:value-of select="@no"/></b></font></td>
								</tr>
							</table>
							<tr style="display:none;"><xsl:attribute name="class">panel13<xsl:value-of select="$num33"/></xsl:attribute>
								<td colspan="5">
								<div style="display:none;"><xsl:attribute name="id">panel13<xsl:value-of select="$num33"/></xsl:attribute>
								<table border="1" align="center" class="test1" >
									<tr bgcolor="#DCDCDC">
										<th><font face="Calibri" size="2" color="#000000">Step No</font></th>
										<th><font face="Calibri" size="2" color="#000000">Step Name</font></th>
